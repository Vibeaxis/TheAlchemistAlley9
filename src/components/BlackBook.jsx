...
const QUEST_STEPS = [
  ...
  },
  {
    ...
    requirement: 'Brew 3 Toxic Potions', // You'd track this in stats
...
const BlackBook = ({ isOpen, onClose, discoveredIngredients, brewHistory, gameStats, reputation }) => {
  const [activeTab, setActiveTab] = useState('quest'); // 'quest' | 'ingredients' | 'history'
  ...
  // Mock stats for quest checking if not passed fully
  const currentStats = {
      ...
      <div className="flex border-b border-amber-900/20 bg-[#1e1a16]">
        {[
            ...
            <button
                ...
                ${activeTab === tab.id 
                    ? 'text-amber-200 bg-amber-950/30' 
                    : 'text-amber-900/60 hover:text-amber-500 hover:bg-amber-950/10'}`}
            >
                <tab.icon className="w-4 h-4" /> 
                {tab.label}
                {activeTab === tab.id && (
                    <motion.div layoutId="tab-underline" className="absolute bottom-0 left-0 w-full h-0.5 bg-amber-500" />
                )}
            </button>
        ))}
      </div>
                    ...
                    return (
                        <div key={step.id} className={`relative ${locked ? 'opacity-50 grayscale' : 'opacity-100'}`}>
                            ...
                            <div className={`absolute -left-[21px] top-1 w-3 h-3 rounded-full border-2 
                                ${completed ? 'bg-amber-500 border-amber-500' : 'bg-[#1a1614] border-amber-900/40'}`} 
                            />
                            
                            <div className="flex justify-between items-start mb-1">
                                <h4 className={`text-lg font-bold ${completed ? 'text-amber-200 line-through decoration-amber-900/50' : 'text-amber-100'}`}>
                                    {step.title}
                                </h4>
                            ...
                            <p className="text-amber-700/80 text-sm mb-3 leading-relaxed">
                                {step.description}
                            </p>
                            
                            <div className="bg-black/20 p-3 rounded border border-amber-900/20 flex flex-col gap-2">
                                <div className="flex justify-between text-xs font-mono uppercase tracking-wider text-amber-600">
                                    <span>Goal:</span>
                                    <span>{step.requirement}</span>
                                </div>
                                <div className="flex justify-between text-xs font-mono uppercase tracking-wider text-emerald-600/80">
                                    <span>Reward:</span>
          ...
          <div className="grid grid-cols-1 gap-4">
            {INGREDIENTS.map((ing) => {
              ...
              return (
                <div 
                  key={ing.name}
                  className={`relative p-4 rounded border transition-all overflow-hidden group
                    ${isDiscovered 
                      ? 'bg-[#241f1a] border-amber-900/30 hover:border-amber-500/30' 
                      : 'bg-[#161210] border-transparent opacity-60'}`}
                >
                  <div className="flex justify-between items-start">
                      <div className="flex items-center gap-4">
                        <div className={`text-4xl w-12 h-12 flex items-center justify-center bg-black/20 rounded border border-white/5 
                            ${!isDiscovered && 'filter blur-sm grayscale brightness-50'}`}>
                            {ing.icon}
                        </div>
                        <div>
                            <h3 className={`font-bold text-lg ${isDiscovered ? 'text-amber-200' : 'text-amber-900'}`}>
                                {isDiscovered ? ing.name : 'Unknown Substance'}
                            </h3>
                            {isDiscovered && (
                                <p className="text-xs text-amber-700 italic mt-0.5">
                                    "A potent reagent found in the depths..."
                                </p>
                            )}
                        </div>
                      </div>
                  ...
                  {isDiscovered ? (
                      <div className="mt-4 flex flex-wrap gap-2">
                          {ing.tags.map((tag) => (
                              <span key={tag} className="text-[10px] font-bold uppercase tracking-wider px-2 py-1 bg-black/30 text-amber-500/80 rounded border border-amber-900/20">
                                  {tag}
                              </span>
                          ))}
                          {ing.processed && (
                      ...
                      <div className="mt-4 text-xs text-amber-900/40 uppercase tracking-widest font-mono">
                          Requires Discovery
                      </div>
                  )}
                </div>
          ...
          <div className="space-y-3 font-mono text-sm">
            {brewHistory.length === 0 ? (
              ...
              brewHistory.map((entry) => (
                <div 
                  key={entry.id}
                  className={`p-3 rounded border-l-2 bg-[#1e1a16] border-amber-900/10 hover:bg-[#241f1a] transition-colors`}
                >
                  <div className="flex justify-between items-center mb-2 text-[10px] text-amber-900/60 uppercase tracking-widest">
                     <span>Day {entry.day}</span>
                     <span>{new Date(entry.id).toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'})}</span>
                  </div>
                  
                  <div className="flex justify-between items-start mb-2">
                     <span className="text-amber-200/90 font-bold">{entry.customerClass}</span>
                     <div className={`flex items-center gap-1.5 text-[10px] font-bold uppercase px-2 py-0.5 rounded ${getResultColor(entry.result)}`}>
                  ...
                  <div className="flex gap-4 text-xs border-t border-white/5 pt-2 mt-2">
                      <span className={entry.goldChange > 0 ? 'text-emerald-500/80' : 'text-slate-500'}>
                        {entry.goldChange > 0 ? '+' : ''}{entry.goldChange} Gold
                      </span>
                      <span className={entry.repChange > 0 ? 'text-blue-400/80' : 'text-red-400/80'}>
